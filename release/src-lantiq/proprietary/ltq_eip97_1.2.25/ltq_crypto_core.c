...
unsigned int PEC_ResultNotificationCount = 0;
...
static inline u32 cryto_mode_get(struct crypto_tfm *tfm, int is_hash)
{
	if (is_hash)
...
/* Get the crypto type */
static inline u32 crypto_flag_get(struct crypto_tfm *tfm, int is_hash)
{
	if (is_hash)
		return container_of(__crypto_ahash_alg(tfm->__crt_alg),
...
/* Get the hash type */
static inline u32 hash_mode_get(struct crypto_tfm *tfm, int is_hash)
{
	if (is_hash)
		return container_of(__crypto_ahash_alg(tfm->__crt_alg),
...
static int sg_size_check(struct scatterlist *sg)
{
...
/* Issues seen for packet byte less than 3B, so we do chanining */
static void ltq_gather_buffer(u8 *sgbuf, struct scatterlist *sg, 
							unsigned int nbytes)
{
	int i;

	for (i = 0; i < nbytes; sg = sg_next(sg)) {
		memcpy(sgbuf, sg_virt(sg), sg->length);
		sgbuf += sg->length;
		i += sg->length;
	}
}

/* unchain the buffers back to the scatter lists */
static int ltq_scatter_buffer(u8 *buf, struct scatterlist *sg, 
							unsigned int nbytes)
{
	int i;
	u8 *tmpbuf = buf;
	...
	for (i = 0; i < nbytes; sg = sg_next(sg)) {
		memcpy(sg_virt(sg), tmpbuf, sg->length);
		tmpbuf += sg->length;
		i += sg->length;
	}
	kfree(buf);
...
static void ltq_pkt_done(PEC_ResultDescriptor_t *res, int result, int dev)
{
	...
	case CRYPTO_ALG_TYPE_ABLKCIPHER: {
		struct ablkcipher_request *areq = (struct ablkcipher_request *) ctrl->data.ablk_req;
    	struct crypto_ablkcipher *ablk_tfm = crypto_ablkcipher_reqtfm(areq);
		...
		if (ctrl->is_chained) {
			ltq_scatter_buffer(ctrl->copybuf, areq->dst, areq->nbytes);
		}
	...
	case CRYPTO_ALG_TYPE_AEAD: {
		struct aead_request *aead = (struct aead_request *) ctrl->data.aead_req;
								...
								unsigned int particles)
{
	...
	for (i = 0; i < particles; i++) { 
		...
		tmp_sg = scatterwalk_sg_next(tmp_sg);
	}
						...
						u32 *ipad, u32 *opad, u32 *auth_key3)
{
    ...
    crypto_flag = crypto_flag_get(tfm, 1);
	crypto_mode = cryto_mode_get(tfm, 1);
	auth_flag = hash_mode_get(tfm, 1);
...
static int ltq_setup_aead_assoc(struct crypto_tfm *tfm,
						...
						u32 *ipad, u32 *opad)
{
	...
	crypto_flag = crypto_flag_get(tfm, 0);
	crypto_mode = cryto_mode_get(tfm, 0);
	auth_flag = hash_mode_get(tfm, 0);
	...
	if (crypto_flag != SAB_CRYPTO_NULL && 
		...
		if (dir) {
			/* Incoming packets */
			struct ltq_aead_ctx *ctx = crypto_tfm_ctx(tfm);
			SAParamsBasic_p.ICVByteCount = ctx->authsize;
		} else {
			/* outgoing packets */
			SAParamsBasic_p.ICVByteCount = ctx->authsize;
						...
						void (*callback) (void *data))
{
	...
	if ((req->src->length >=  nbytes)) {
		...
		/* corner case for scatter less than 2 bytes */
		if (sg_size_check(req->src)) {
			ctrl->is_chained = 1;
			src = (u8 *) kmalloc(nbytes, GFP_KERNEL);
			if (!src) {
				pr_err("error copying scatter buffer\n");
				rc = -ENOMEM;
				goto cipher_err;
			}
			ltq_gather_buffer(src, req->src, nbytes);
			ctrl->copybuf = src;
			dma_alloc_properties(&DMAProperties, 
						true, nbytes, 
						LTQ_ALIGN, 
			...
			DMABuf_Register(DMAProperties, src,  NULL, 'k', &ctrl->DstPktHandle);
    		if (DMAStatus != DMABUF_STATUS_OK) {
        		pr_err("%s: Failed to register src DMA buf\n", __func__);
				rc = -ENOMEM;
				goto cipher_err;
			...
			rc = ltq_buffers_chaining(ctrl, req->src, 
								&ctrl->SrcPktHandle,
			...
			rc = ltq_buffers_chaining(ctrl, req->dst, 
								&ctrl->DstPktHandle,
	...
	if (!ctx->registered) {
		CBFunc = (PEC_NotifyFunction_t) callback;
		rc = PEC_ResultNotify_Request(EIP97_RING_ENC, CBFunc, 1);
		if (rc != PEC_STATUS_OK) {
...
static int ltq_rfc3686_encrypt(struct ablkcipher_request *req)
{
    ...
    *(__be32 *)(iv + CTR_RFC3686_NONCE_SIZE + CTR_RFC3686_IV_SIZE) =
	...
	ret = ltq_process_cipher(req, ctrl, 0, EIP97_ENC_CALLBACK);
	req->info = info;
	return ret;
}
...
static int ltq_rfc3686_decrypt(struct ablkcipher_request *req)
{
    ...
    *(__be32 *)(iv + CTR_RFC3686_NONCE_SIZE + CTR_RFC3686_IV_SIZE) =
	...
	ctrl->data.ablk_req = req;
	ret = ltq_process_cipher(req, ctrl, 1, EIP97_ENC_CALLBACK);
	req->info = info;
	return ret;
}
...
static int ltq_ablk_encrypt(struct ablkcipher_request *req)
{
	...
	ctrl->is_chained = 0;
	ctrl->data.ablk_req = req;
	return ltq_process_cipher(req, ctrl, 0, EIP97_ENC_CALLBACK);
}
...
static int ltq_ablk_decrypt(struct ablkcipher_request *req)
{
	...
	ctrl->is_chained = 0;
	ctrl->data.ablk_req = req;
	return ltq_process_cipher(req, ctrl, 1, EIP97_ENC_CALLBACK);
}
							...
							void (*callback) (void *data))
{
	...
	DMABuf_Register(DMAProperties, req->result,  NULL, 'k', &ctrl->DstPktHandle);
    if (DMAStatus != DMABUF_STATUS_OK) {
       	pr_err("%s: Failed to register dst DMA buf\n", __func__);
		rc = -EAGAIN;
		goto ahash_err;
                                ...
                                const u8 *key, unsigned int keylen)
{
    ...
    ablk_tfm = crypto_alloc_ablkcipher(algo, 0, 0);
    if (IS_ERR(ablk_tfm)) {
        pr_err("Error allocating ablkcipher transform\n");
        return PTR_ERR(ablk_tfm);
    }

    data = kzalloc(sizeof(*data) + crypto_ablkcipher_reqsize(ablk_tfm),
               GFP_KERNEL);
    if (!data)
        return -ENOMEM;

    data->req = ablkcipher_request_alloc(ablk_tfm, GFP_KERNEL);
    if (!data->req) {
        pr_err("Error allocating request for ablkcipher tfm\n");
        goto out;
    }
    crypto_ablkcipher_clear_flags(ablk_tfm, CRYPTO_TFM_REQ_MASK);

    err = crypto_ablkcipher_setkey(ablk_tfm, key, keylen);
    if (err)
        return err;
    ...
    ablkcipher_request_set_tfm(data->req, ablk_tfm);
...
out:
    kfree(data);
    crypto_free_ablkcipher(ablk_tfm);
    return 0;
}
									...
									unsigned int len))
{
	...
	pr_debug("alg: %s, ds: %d, ss: %d\n", 
			hash->base.__crt_alg->cra_name, ds, ss);
              ...
              unsigned int keylen)
{
	...
	int ret;
	u32 hash_flag = hash_mode_get(tfm, 1);
	...
	ret = ltq_setup_ahash_assoc(tfm, ctx->ipad, ctx->opad, ctx->auth_key3);
					...
					char *base_hash_name)
{
	...
	pr_debug("shash name: %s\n", shash->base.__crt_alg->cra_name);
	return shash;
}
...
static int ltq_ahash_init(struct ahash_request *req)
{
	...
	if (req->nbytes == 0) {
		/* maybe we should have a fallback algo until the API supports it */
		if (hash_flag == SAB_AUTH_AES_XCBC_MAC)
			return -EINVAL;
	...
	ctx->ds = ds;
	ctx->TokenFlags = 0;
	ctx->TokenFlags |= (TKB_PACKET_FLAG_HASHFIRST | TKB_PACKET_FLAG_HASHFINAL);
	ctrl->SA_flag = alg->cra_flags;
	ctrl->data.req = req;
	...
	return ltq_process_ahash(ctx, ctrl, req, EIP97_ENC_CALLBACK);
}
					...
					void (*callback)(void *data))
{
	...
	unsigned int tries = 5;
	unsigned ivsize;
	u8 *src_buf, *dst_buf, *assocbuf;
    u32 hash_alg = hash_mode_get(cryptfm, 0);
    void *TCRDataOut = (void *) ctx->TCRDataOut;	
	...
	if (sg_cnt == 1) {
        ...
        DMABuf_Register(DMAProperties, src_buf, NULL, 'k', &ctrl->SrcPktHandle);
        if (DMAStatus != DMABUF_STATUS_OK) {
            pr_err("%s: Failed to register Src DMA buf\n", __func__);
			rc = -EAGAIN;
            goto aead_err; 
        ...
        DMABuf_Register(DMAProperties, dst_buf,  NULL, 'k', &ctrl->DstPktHandle);
        if (DMAStatus != DMABUF_STATUS_OK) {
            pr_err("%s: Failed to register dst DMA buf\n", __func__);
			rc = -EAGAIN;
            goto aead_err;
	...
	if (!ctx->registered) {
		CBFunc = (PEC_NotifyFunction_t) callback;
		rc = PEC_ResultNotify_Request(cur_ring, CBFunc, 1);
		if (rc != PEC_STATUS_OK) {
			pr_err("Unable to register IRQ handler\n");
			rc = -EINVAL;
			goto aead_err;
	...
	if (hash_alg == SAB_AUTH_AES_GCM ||
		(hash_alg == SAB_AUTH_AES_GMAC) ||
		(hash_alg == SAB_AUTH_AES_CCM)) {
		TokenParams.IV_p = iv;
        if (assoclen > 0) {
			if (hash_alg == SAB_AUTH_AES_GMAC) {
				assocbuf = ctrl->assoc_data;
				if (assoclen > 32)
					return -EINVAL;
				/* GMAC expects AAD | IV | payload */
				memcpy(assocbuf, sg_virt(req->assoc), assoclen);
				memcpy(assocbuf + assoclen, req->iv, ivsize);
				TokenParams.AAD_p = assocbuf;
            	TokenParams.AdditionalValue = assoclen + ivsize;
			} else {
				TokenParams.AAD_p = sg_virt(req->assoc);
		...
		TokenParams.IV_p = iv;
	    if (direction == 0) {
    	    if (assoclen > 0 && assoclen < 64) {
				assocbuf = ctrl->assoc_data;
    	        memcpy(assocbuf, sg_virt(req->assoc), assoclen);
       	     	memcpy(assocbuf + assoclen, iv, ivsize);
       	     	TokenParams.AAD_p = assocbuf;
            	TokenParams.AdditionalValue = assoclen + ivsize;
        	}
        	else {
            	TokenParams.AAD_p = iv;
            	TokenParams.AdditionalValue = ivsize;
        	}
    	} else {
        	TokenParams.AAD_p = sg_virt(req->assoc);
        	TokenParams.AdditionalValue = assoclen;
                  ...
                  unsigned int keylen)
{
	...
	u32 hash_flag = hash_mode_get(tfm, 0);
	void (*func)(u32 *ipad, u32 *opad,
				void *istate,
				void *ostate,
				unsigned int len); 
	...
	memcpy(&ctx->authkey, key, authkeylen);
	memcpy(&ctx->enckey, key + authkeylen, enckeylen);
...
static int ltq_aead_cra_init(struct crypto_tfm *tfm)
{
	struct ltq_aead_ctx *ctx =  crypto_tfm_ctx(tfm);
	struct crypto_shash *shash = NULL;
	u32 hash_flag = hash_mode_get(tfm, 0);
...
static void ltq_aead_cra_exit(struct crypto_tfm *tfm)
{
	struct ltq_aead_ctx *ctx = crypto_tfm_ctx(tfm);
...
static int ltq_aead_encrypt(struct aead_request *req)
{
	...
	if (!ctx->config_sa_done) {
		ret = ltq_setup_aead_assoc(tfm, ctrl, ctx, 
	...
	return ltq_process_aead(req, ctrl, req->src,
					req->dst, req->assoc, LTQ_ENC, req->iv,
					EIP97_ENC_CALLBACK);
}
...
static int ltq_aead_decrypt(struct aead_request *req)
{
	...
	ctrl->SA_flag = ctx->SA_flag;
	return ltq_process_aead(req, ctrl, req->src,
					req->dst,req->assoc, LTQ_DEC, req->iv,
					EIP97_DEC_CALLBACK);
}
...
static int ltq_aead_givencrypt(struct aead_givcrypt_request *req)
{
	...
	struct ltq_crypt_ctrl *ctrl;
	int ret;
    unsigned len, ivsize = crypto_aead_ivsize(authenc);
    __be64 seq;
	...
	ctx->TokenFlags = TKB_PACKET_FLAG_HASHAPPEND;
	ctrl->dir = LTQ_ENC;
	ctrl->data.aead_req = &req->areq;
	ctrl->SA_flag = ctx->SA_flag;

	return ltq_process_aead(&req->areq, ctrl, req->areq.src,
					req->areq.dst, req->areq.assoc, LTQ_ENC, req->giv, 
					EIP97_ENC_CALLBACK);
}
...
static int ltq_aead_gcm_transform(struct aead_request *req, int direction)
{
    ...
    ctrl->SA_flag = ctx->SA_flag;
    return ltq_process_aead(req, ctrl, req->src,
                    req->dst, req->assoc, direction, iv,
                    callback);
...
static int ltq_gcm_rfc4xxx_givencrypt(struct aead_givcrypt_request *req)
{
    struct crypto_aead *authenc = aead_givcrypt_reqtfm(req);
    struct ltq_aead_ctx *ctx = crypto_aead_ctx(authenc);
    u8 *iv = ctx->iv;
	...
	memcpy(iv, ctx->nonce, 4);
    memcpy(iv + 4, req->giv, 8);
	*((__be32 *)(iv + 12)) = counter;
    
    return ltq_aead_gcm_transform(&req->areq, LTQ_ENC);
}
...
static int ltq_aead_rfc4309_ccm_decrypt(struct aead_request *req)
{
    ...
    ctrl->SA_flag = ctx->SA_flag;
    return ltq_process_aead(req, ctrl, req->src,
                    req->dst,req->assoc, LTQ_DEC, iv,
                    EIP97_DEC_CALLBACK);
}

static int ltq_aead_rfc4309_ccm_givencrypt(struct aead_givcrypt_request *req)
{
    struct crypto_aead *authenc = aead_givcrypt_reqtfm(req);
    struct crypto_tfm *tfm = crypto_aead_tfm(authenc);
    struct ltq_aead_ctx *ctx = crypto_aead_ctx(authenc);
	...
	return ltq_process_aead(&req->areq, ctrl, req->areq.src,
					req->areq.dst, req->areq.assoc, LTQ_ENC, iv, 
					EIP97_ENC_CALLBACK);
}
...
static struct ltq_crypto_template ltq_crypto_alg[] = {
	...
	{	.type = CRYPTO_ALG_TYPE_ABLKCIPHER,
		.alg.crypto = {
			.cra_name = "ctr(aes)",
			.cra_driver_name = "ctr-aes-ltq-crypto",
			.cra_blocksize = AES_BLOCK_SIZE,
			.cra_flags = CRYPTO_ALG_TYPE_ABLKCIPHER |
							CRYPTO_ALG_ASYNC,
			.cra_type        = &crypto_ablkcipher_type,
	...
	},
	{
        ...
        .alg.crypto = {
            ...
            .cra_aead = {
				...
				.givencrypt = ltq_gcm_rfc4xxx_givencrypt,
            }
        },
        .auth_flag = SAB_AUTH_AES_GCM,
        .crypto_flag = SAB_CRYPTO_AES,
    ...
    },	
    {
        ...
        .alg.crypto = {
            ...
            .cra_aead = {
				...
				.givencrypt = ltq_aead_rfc4309_ccm_givencrypt,
            }
        },
        .auth_flag = SAB_AUTH_AES_CCM,
        .crypto_flag = SAB_CRYPTO_AES,
...
static int __init ltq_crypto_init(struct platform_device *pdev)
{
    ...
    ltq_crypto_membase = devm_ioremap_nocache(&pdev->dev, res->start,
                        resource_size(res));
	...
	for (i = 0; i < ARRAY_SIZE(ltq_crypto_alg); i++) {
		...
		case CRYPTO_ALG_TYPE_AHASH:
			...
			tmpl->alg.hash.halg.statesize = sizeof(struct ltq_ahash_ctx)/8;
			tmpl->alg.hash.init = ltq_ahash_init;
			tmpl->alg.hash.update = ltq_ahash_update;
			tmpl->alg.hash.final = ltq_ahash_final;
			if (!tmpl->alg.hash.setkey)
				tmpl->alg.hash.setkey = ltq_ahash_setkey;
			tmpl->alg.hash.digest = ltq_ahash_digest;
			break;
		case CRYPTO_ALG_TYPE_AEAD:
			t_alg = &tmpl->alg.crypto;
			t_alg->cra_type = &crypto_aead_type;
			t_alg->cra_ctxsize = sizeof(struct ltq_aead_ctx);
			t_alg->cra_init = ltq_aead_cra_init;
			t_alg->cra_exit = ltq_aead_cra_exit;
            if (!tmpl->alg.crypto.cra_aead.setkey)
                tmpl->alg.crypto.cra_aead.setkey = ltq_aead_setkey;
            if (!tmpl->alg.crypto.cra_aead.setauthsize)
                tmpl->alg.crypto.cra_aead.setauthsize = ltq_aead_setauthsize;
            if (!tmpl->alg.crypto.cra_aead.encrypt)
                tmpl->alg.crypto.cra_aead.encrypt = ltq_aead_encrypt;
            if (!tmpl->alg.crypto.cra_aead.decrypt)
                tmpl->alg.crypto.cra_aead.decrypt = ltq_aead_decrypt;
            if (!tmpl->alg.crypto.cra_aead.givencrypt) 
                tmpl->alg.crypto.cra_aead.givencrypt = ltq_aead_givencrypt;
			break;
		default: 
		...
		if (tmpl->type == CRYPTO_ALG_TYPE_ABLKCIPHER || 
			tmpl->type == CRYPTO_ALG_TYPE_AEAD) {
