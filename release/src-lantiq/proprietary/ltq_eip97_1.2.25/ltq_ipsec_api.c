...
static struct ltq_ipsec_complete ipsec_done[MAX_DESC_LIST_SIZE];
u32 cdr_fifo[MAX_DESC_LIST_SIZE * CDR_DESC_SIZE];
u32 rdr_fifo[MAX_DESC_LIST_SIZE * RDR_DESC_SIZE];
...
static void get_packet(unsigned long data)
{
	int err;
	int i, no_pkt_read;
	unsigned int ret_pkt_len, max_read;
	u32 *rdr_ptr;
	u32 reg;
	struct ltq_ipsec_complete *done;
#ifndef IOCU_MODE
	...
	Device_Write32(Global_Base, RDR_STAT_OFFSET(cur_ring), 0x10);
	pr_debug(" #########  No pkt done processed: %d, reading: %d\n", no_pkt_read, max_read);
	...
	for (i = 0; i < max_read; i++) {
		...
		rdr_ptr = __va(DMAaddr);
#else
		rdr_ptr =  &rdr_fifo[rdr_desc_offset * RDR_DESC_SIZE];
#endif
		...
		if (done->ret_pkt_len >= 0x800) {
			pr_err("@%s: Error processing packet @ rdr offset %d\n", 
						__func__, rdr_desc_offset);
			err = -EINVAL;
		}
...
static irqreturn_t ipsec_handler(int irq,
								void *data)
{
...
static struct ltq_crypto_ipsec_params *get_params(u32 spi) 
{
	struct list_head *ptr;
	struct ipsec_list *entry;
	...
	pr_err("Cannot find SPI entry 0x%08x\n", spi);
	return NULL;
...
static int eip97_crypto_api(struct ltq_crypto_ipsec_params *req, u32 *tokenbuf,
				u8 *in, u8 *out, void (*callback)(struct ltq_ipsec_complete *done),
				...
				unsigned int padlen, int direction)
{
	...
	cdr_ptr = &cdr_fifo[desc_offset * CDR_DESC_SIZE];
	rdr_ptr = &rdr_fifo[desc_offset * RDR_DESC_SIZE];

	done_ptr = &ipsec_done[desc_offset];
			...
			unsigned int buflen, void *ip_data) 
{
	...
	if (req->total_pad_offs && req->total_pad_offs < req->tokenwords) {
		...
		if (req->ip_type == SAB_IPSEC_IPV4) {
			set_val(token[req->total_pad_offs], ESP_NH_IP, INSERT_XLIV_MASK, INSERT_XLIV_OFFSET);
		} else {
			set_val(token[req->total_pad_offs], ESP_NH_IP6, INSERT_XLIV_MASK, INSERT_XLIV_OFFSET);
		}
	}
	...
	ret = eip97_crypto_api(req, token, in, out, callback,
...
int ltq_ipsec_init(unsigned int ring, int irq)
{
	...
	u32 reg = 0;
	u32 *cdr_ptr = cdr_fifo; 
	u32 *rdr_ptr = rdr_fifo; 
