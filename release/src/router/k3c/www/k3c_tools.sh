...
do
	...
	if [ "$k3c_warnning" != "1" ];then
		if [ ! -d "/jffs/softcenter" ] ;then
			...
			#wget --no-check-certificate --timeout=10 --tries=3 -qO /tmp/softcenter.zip http://k3c.paldier.com/tools/softcenter.zip
				#if [ "$?" == "0" ]; then
					#unzip -o /tmp/softcenter.zip -d /tmp/softcenter
