...
#define K3CLED_DIR	"/sys/class/leds/wan"
#define K3CMAC1	"00:11:22:33:44:55"
#define K3CMAC2	"00:11:22:33:44:58"
...
void k3c_load(char result[17])
{
			char *buffer = read_whole_file("/tmp/k3cinit");

			if (buffer) {
				sscanf(buffer, "%s", result);
				free(buffer);
			} else {
				strcpy(result,K3CMAC1);
			}
			unlink("/tmp/k3cinit");
			doSystem("rm -rf /tmp/k3cinit");
			//_dprintf("k3c:%s\n\n\n\n\n", result);
			//return result;
}

void k3c_init_syspara(char mac1[], char mac2[])
{
	...
	if (FRead(buffer, 0x7ec1002, bytes) < 0) {  // ET0/WAN is same as 2.4G
		_dprintf("k3c : Out of scope\n");
	} else {
		if (buffer[0] != 0xff)
			{
			ether_etoa(buffer, mac1);
			}
	}

	if (FRead(buffer, 0x7ec5006, bytes) < 0) { // ET1/LAN is same as 5G
		_dprintf("k3c : Out of scope\n");
	} else {
		if (buffer[0] != 0xff)
			{
			ether_etoa(buffer, mac2);
			}
	}
}


void k3c_pre_init_wifi()
{
		doSystem("cp -s /opt/lantiq/wave/images/backup/cal_wlan0.bin /tmp/cal_wlan0.bin");
		doSystem("cp -s /opt/lantiq/wave/images/backup/cal_wlan2.bin /tmp/cal_wlan1.bin");
		doSystem("cp -s /opt/lantiq/wave/images/backup/cal_wlan2.bin /tmp/cal_wlan2.bin");
}
void k3c_insmod(){
...
void k3c_init()
{
	...
	_dprintf("k3c:init system env\n");
	doSystem("nvram set bl_ver=`uboot_env --get --name ver`");
	doSystem("nvram commit");
	doSystem("nvram set k3cmac=`uboot_env --get --name ethaddr`");
	strcpy(mac3,nvram_get("k3cmac"));
	k3c_init_syspara(mac1, mac2);
	ether_atoe(mac1, mac_binary);
	ether_atoe(mac2, mac_binary2);
	if((mac_binary[5] +4 == mac_binary2[5]) && (strcmp(mac1, mac3) == 0))
	{
		_dprintf("k3c:init system env done\n");
		//doSystem("logger -t 'K3C' '*** init system env done ***'");
		nvram_set_int("success_init_k3c",0);
		//nvram_set_int("success_start_k3c", 0);
		nvram_commit();
	} else {
		_dprintf("k3c:init system env failed, Reboot after 1 hour\n");
		//doSystem("logger -t 'K3C' '*** init system env failed, Reboot after 1 hour, Fix your router firmware please ***'");
		nvram_set_int("success_init_k3c",1);
		//nvram_set_int("success_start_k3c", 1);
		nvram_commit();
	}
...
{
		pid_t pid;
		char *argv[]={"/sbin/delay_exec","600","reboot",NULL};
...
{
	_dprintf("############################ K3C init done #################################\n");
	if (nvram_get_int("success_init_k3c") == 1){
		logmessage("K3C", "*** init system env failed, Reboot after 10 min, Fix your router firmware please ***\n");
		k3c_failed();
	} else
		logmessage("K3C", "*** init system env done ***\n");
#if defined(MERLINR_VER_MAJOR_R) || defined(MERLINR_VER_MAJOR_X)
#if defined(K3C)
	if (auth_code_check(nvram_get("et0macaddr"), nvram_get("uuid")) == 1)
...
#endif
	{
		logmessage("K3C", "*** verify done ***\n");
	} else {
		logmessage("K3C", "*** verify failed, Reboot after 10 min ***\n");
	}
#endif
	k3c_pre_init_wifi();
	nvram_unset("k3cmac");
	logmessage("K3C", "uuid:%s", nvram_get("uuid"));
	logmessage("K3C", "mac:%s", nvram_get("et0macaddr"));
#if defined(RTCONFIG_SOFTCENTER)
...
int merlinr_firmware_check_update_main(int argc, char *argv[])
{
	...
	//char *nt_center=nvram_get("rc_support");
	char info[100];
